...
function HeroSection(){
    ...
    const handleContactMe = () => {
        window.location.href = `mailto:${data.email}?subject=Let's Connect&body=Hi Saroj, I would like to connect with you.`;
    };
    ...
    return(
        <div>
            <div id="hero" className="flex  flex-col items-center gap-y-2 md:gap-y-3 mt-4 md:mt-6 px-4">
                ...
                {/* Name heading */}
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold pb-2 text-center">{data.name}</h1>
                ...
                {/* Action buttons */}
                <div className="flex flex-row  md:flex-row justify-center gap-3 md:gap-6 w-full max-w-xs md:max-w-none mt-2">
                    <button 
                        onClick={handleResumeDownload}
                        className="flex items-center  justify-center md:justify-start px-4 md:px-6 py-2 md:py-3 border-1 border-white rounded-lg hover:cursor-pointer hover:bg-white hover:border-indigo-900 hover:text-black transition w-full md:w-auto"
                    >
                        <span className="mr-2 text-xl md:text-2xl"><IoMdDownload /></span> Download Resume
                    </button>
                    <button 
                        onClick={handleContactMe}
                        className="flex items-center justify-center md:justify-start px-4 md:px-6 py-2 md:py-3 bg-transparent border border-white hover:cursor-pointer hover:bg-white hover:border-indigo-900 hover:text-black rounded-lg transition w-full md:w-auto"
                    >
